    ...
    <section id="top" className="relative overflow-hidden pt-32 pb-20 sm:pt-40 sm:pb-28">
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <h1 className="reveal mx-auto max-w-4xl text-center font-display text-4xl font-800 leading-[1.08] tracking-tight text-white sm:text-6xl lg:text-7xl">
          ...
          <span className="text-gradient">5 figures per month</span> in under 5 hours a week.
        </h1>

        <p className="reveal mx-auto mt-7 max-w-2xl text-center text-base leading-relaxed text-ink-300 sm:text-lg">
          Private 1-on-1 mentorship that installs the exact entries, risk rules and exits, so you
          can compound while keeping your job. Capped intake, by application.
        </p>
        ...
        <div className="reveal mx-auto mt-12 max-w-3xl">
          <button
            ...
            className="group relative block aspect-video w-full overflow-hidden rounded-2xl border border-white/10 bg-ink-900 text-left shadow-2xl shadow-black/40"
            aria-label="Continue to the mentorship application"
          >
            <img
              src="/youtube_4k_polished.png"
              alt="Crypto trading mentorship preview"
            ...
            <span className="absolute bottom-4 left-5 flex items-center gap-2 text-sm font-500 text-ink-200">
              <span className="rounded bg-red-500 px-1.5 py-0.5 text-[10px] font-700 uppercase tracking-wider text-white">
                Start here
              </span>
              See how it works — then apply
            </span>
          </button>
        </div>
            ...
            {TRACKS.map((t) => {
              ...
              return (
                ...
                >
                  <div className="mb-3 flex items-center justify-between">
                    <span
                      className={`flex h-10 w-10 items-center justify-center rounded-xl transition-colors ${
                        active ? 'bg-accent-500 text-ink-950' : 'bg-white/5 text-accent-400'
                      }`}
                    >
                  ...
                  <h3 className="font-display text-base font-700 text-white">{t.title}</h3>
                  <p className="mt-1 text-xs leading-relaxed text-ink-300">{t.desc}</p>
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-5 text-xs text-ink-400">
              <span className="flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 text-accent-400" /> Education only
              </span>
              <span className="flex items-center gap-1.5">
                <Star className="h-3.5 w-3.5 text-gold-400" /> Reviewed within 2 business days
              </span>
            </div>
          </div>
        </div>
...
const TRACKS = [
  { id: 'new', title: 'New to crypto', desc: 'Starting from zero. Want a clear, repeatable process.', icon: Sprout },
  { id: 'part-time', title: 'Part-time trader', desc: 'Have a job. Want to compound without quitting.', icon: Clock },
  { id: 'full-time', title: 'Full-time trader', desc: 'Trading now but results are inconsistent.', icon: Briefcase },
  { id: 'scaling', title: 'Scaling up', desc: 'Already profitable. Want tighter risk and size.', icon: Rocket },
];
